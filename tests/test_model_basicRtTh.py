...
def test_steady_Ksp_no_precip_changer(clock_simple, grid_2, depression_finder, m_sp, n_sp):
    U = 0.0001
    ...
    rock_predicted_slopes_lower = ((U + 0.) / (Kr * (actual_areas ** m_sp))) ** (
        1. / n_sp
    )
    till_predicted_slopes_lower = ((U + 0.) / (Kt * (actual_areas ** m_sp))) ** (
        1. / n_sp
    )
