...
def test_run_stochastic_opt_true(clock_04, grid_1):
    params = {
        ...
        "m_sp": 0.5,
        "n_sp": 1.0,
        "water_erodability_stochastic": 0.01,
...
def test_run_stochastic_opt_false(clock_05, grid_1):
    ...
    assert (
        np.array_equiv(
            dry_times, model.clock.step * (1. - params["rainfall_intermittency_factor"])
        )
        is True
    )
    assert (
        np.array_equiv(
            wet_times, model.clock.step * (params["rainfall_intermittency_factor"])
        )
        is True
    )
