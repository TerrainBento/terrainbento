...
@pytest.mark.parametrize("intervals, times, output_times", [
    ...
    (5.0, None, [0, 5, 10, 15, 20, None]), # Single interval duration (float)
...
@pytest.mark.parametrize("intervals, times, output_times", [
    ...
    ([1.0,2.0,3.0], None, [1, 3, 6, 20, None]), # List of float intervals
    ...
    (None, [1.0,2.0,3.0,20.0], [1, 2, 3, 20, None]), # List of float times

    # Test intervals that jump the stop time
    (6  , None, [6, 12, 18, 20, None]), # Single interval jumps stop time
    ([1,2,3,50], None, [1, 3, 6, 20, None]), # List intervals jumps stop time
    ])
...
@pytest.mark.parametrize("save_last", [
    (False),
    (True),
    ])
